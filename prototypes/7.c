...
main(void)
{
	...
	for (i = 0; i < 10; i++)
	{
		...
		chunk_size = *((size_t *) (mem_info + 0x08));
		free_flag = chunk_size & 1;
		chunk_size -= free_flag;
		prev_chunk_size = *((size_t *) mem_info);

		printf("chunks[%d]: %p, size = %li, prev (%s) = %li\n",
			i, mem_info + 0x10, chunk_size,
		 	(free_flag? "allocated": "unallocated"),
			prev_chunk_size);
	}
